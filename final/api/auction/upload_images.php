...
if (!empty($_POST['token']) || !$_SESSION['token']) {
  if (hash_equals($_SESSION['token'], $_POST['token'])) {
    ...
    if(!$productId){
      $reply = array('error' =>  "Invalid product!");
      echo json_encode($reply);
      return;
    }

    if(getAuctionState($auctionId) != 'Created'){
      $reply = array('error' => "The auction has already started. You can't update it anymore.");
      echo json_encode($reply);
      return;
    }
    ...
    if($nrImages > 10 || $nrImages < 1){
      $reply = array('error' =>  print_r($_POST));
      echo json_encode($reply);
      return;
    }
    ...
    if(count($productImages) >= 10){
      $reply = array('error' =>  "You can't upload more images. The maximum number of images was achieved!");
      echo json_encode($reply);
      return;
    }

    if(count($productImages) + $nrImages > 10){
      $reply = array('error' =>  "You can't upload so many images. The maximum number of images per product is 10!");
      echo json_encode($reply);
      return;
    }
    ...
    if(count($captionsArr) != $nrImages){
      $reply = array('error' =>  "Every picture must have a caption.");
      echo json_encode($reply);
      return;
    }
    ...
    foreach ($captionsArr as $caption){
      if($caption == ""){
        ...
        $reply = array('error' =>  "Every picture must have a caption with a maximum length of 128.");
        echo json_encode($reply);
        return;
      }
    }
